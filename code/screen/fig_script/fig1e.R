...
marker <- markers[2]
print(marker)
b_marker_raw_gene_tests_file <- paste0(test_dir,"/",gene_files[2])
b_marker_raw_gene_tests_table <- read.table(b_marker_raw_gene_tests_file,sep="\t",header=1) %>% rename(sgrna_gene = id)
b_marker_raw_tests_file <- paste0(test_dir,"/",files[2])
b_marker_raw_tests_table <- read.table(b_marker_raw_tests_file,sep="\t",header=1)

df_list[[markers[1]]] %>% filter(sgrna_gene=="CD7")

(b_marker_sgrna_gene_tests_table) %>% filter(sgrna_gene=="CD7")
b_marker_sgrna_gene_tests_table <- b_marker_raw_tests_table %>% 
    ...
    mutate(mean_donor_LFC=(r0+r1+r2)/2)
    #mutate(label = ifelse(sgrna_gene %in% sgrna_gene_highlight, sgrna_gene, ""))
head(b_marker_sgrna_gene_tests_table) %>% as.data.frame
...
a_2marker_sgrna_gene_tests_fea_table0 <- Reduce(full_join,df_list)  %>% ungroup %>% 
    ...
    mutate(type = case_when(
          (str_detect("Not a Hit",GFP_P7) & str_detect("Not a Hit",GFP_baseline))  ~ "Not a Hit",
          (str_detect("Positive Hit|Negative Hit",GFP_P7) & str_detect("Not a Hit",GFP_baseline))  ~ "GFP_P7 Only",
          (str_detect("Positive Hit|Negative Hit",GFP_baseline) & str_detect("Not a Hit",GFP_P7))  ~ "GFP_baseline Only",
          (str_detect("Positive Hit|Negative Hit",GFP_P7) & str_detect("Positive Hit|Negative Hit",GFP_baseline))  ~ "Both Screens",
          TRUE  ~ "other")) %>% 
...
a_2marker_sgrna_gene_tests_fea_table <- Reduce(full_join,df_list)  %>% ungroup %>% 
    ...
    mutate(type=factor(type,levels=c("Both Screens","GFP_P7 Only","GFP_baseline Only","Not a Hit"))) %>% 
    mutate(label = ifelse(abs(GFP_P7) > 2.5 | abs(GFP_baseline) > 2.5, sgrna_gene, ""))

a_2marker_sgrna_gene_tests_fea_table %>% filter(sgrna_gene=="CD7")
a_2marker_sgrna_gene_tests_fea_table %>% filter(sgrna_gene=="NR4A1")
a_2marker_sgrna_gene_tests_fea_table$type %>% unique

p_1e_total <-ggplot(a_2marker_sgrna_gene_tests_fea_table, aes(GFP_P7, GFP_baseline)) +
    ...
    theme_bw()+
    labs(x = "log2FoldChange(P4/P7)",y = "log2FoldChange(P4/baseline)", title="CRISPR KO Screens",fill="Screen Hit") +
    scale_fill_manual(values = c("red","green","blue","grey")) +
    scale_color_manual(values = darken(c("red","green","blue","grey"), 0.3))+
    theme(legend.position='right',
          panel.grid =element_blank()) #, ## 删去网格线 panel.border = element_blank())+ ## 删去外层边框

ggsave(p_1e_total,filename="/home/chengennong/ylp/aav/screen/screen_out/batch4/plot/1e.pdf",width=7,height=6)
